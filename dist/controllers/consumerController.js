...
export async function reserve(req, res) {
    ...
    if (slotTime.diff(dayjs(), 'hour') <= 24)
        return res.status(400).json({ error: 'Reservation cutoff passed (24h before)' });
    ...
    if (!cap)
        return res.status(400).json({ error: 'No capacity set for that slot' });
    ...
    if (consumerExisting && consumerExisting.producerId !== producerId)
        return res.status(400).json({ error: 'Only one producer per hour per consumer' });
...
export async function modify(req, res) {
    ...
    if (newKwh < 0.1)
        return res.status(400).json({ error: 'Minimum 0.1 kWh' });
    ...
    if (!cap)
        return res.status(400).json({ error: 'No capacity for slot' });
